...
function KBLogo(): React.JSX.Element {
    return (
...
function IconBtn({ title, onClick, active, children, danger }: IconBtnProps): React.JSX.Element {
...
function NoteItem({ note, active, onLoad, onDelete }: NoteItemProps): React.JSX.Element {
    const [hov, setHov] = useState(false);
    return (
        <div
...
export default function KnexByteEditor(): React.JSX.Element {
...
${content}</body></html>`;
    ...
    }, [notes, currentId, createNote, loadNote]);
    ...
    function cmd(command: string, value: string | null = null) {
                        ...
                        <select
                            onChange={e => {
                                ...
                                document.querySelectorAll('[contenteditable] font[size="7"]').forEach(el => {
                                    (el as HTMLElement).removeAttribute("size"); (el as HTMLElement).style.fontSize = e.target.value + "px";
                                });
                            }}
                            style={{
                                height: 28, background: "#161b22", color: "#8b949e",
