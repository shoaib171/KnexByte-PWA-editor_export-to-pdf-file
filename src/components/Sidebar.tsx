...
const NoteItem = memo(({ note, active, onLoad, onDelete }: NoteItemProps): React.JSX.Element => {
      ...
      {hov && (
        <button
          ...
          style={{
            ...
            background: "var(--kb-item-hover)", border: "1px solid var(--kb-border)",
            color: "var(--kb-danger)", borderRadius: 5, width: 22, height: 22,
...
const Sidebar = memo(({ notes, currentId, onLoadNote, onDeleteNote, onCreateNote }: SidebarProps): React.JSX.Element => {
  return (
    <div style={{
      width: 220, background: "var(--kb-sidebar)", borderRight: "1px solid var(--kb-border)",
      display: "flex", flexDirection: "column", overflow: "hidden", flexShrink: 0
    }}>
      <div style={{ padding: "12px 12px 12px", borderBottom: "1px solid var(--kb-border)" }}>
        <div style={{
          fontSize: 9, fontWeight: 600, color: "var(--kb-text-muted)", textTransform: "uppercase",
          letterSpacing: "1.5px", fontFamily: "'DM Mono',monospace", marginBottom: 12
        }}>Documents</div>
        <button onClick={onCreateNote} style={{
          width: "100%", padding: "8px 10px", background: "transparent",
          border: "1px solid var(--kb-blue)", borderRadius: 8, color: "var(--kb-blue)",
